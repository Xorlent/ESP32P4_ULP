...
public:
     ...
     * software-I2C bus at address 0x44, validates both CRC bytes, stores the latest
        ...
        * low_limit_c_hum > high_limit_c_hum. External pullups on SDA and SCL are
        * required unless i2c_pwr_gpio_num is provided to power the sensor and bus.
        * period_ms must be at least 50 ms.
     */
    ...
    inline bool wakeOnSoftwareI2CSHT4x(uint8_t sda_lp_gpio_num,
                                       uint8_t scl_lp_gpio_num,
                                       int16_t low_limit_c_deg,
                                       int16_t high_limit_c_deg,
